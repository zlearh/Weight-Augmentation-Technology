	...
	def _qu_weight(self,conv,radom):
		...
		if tensor_var == 1:
			transform = transforms.Compose([
				...
				# transforms.RandomAffine(degrees=0,shear=(135, 135)),
		...
		# equality = torch.eq(weights, tra_weight)
		#
		# # 计算相同元素的数量
		# num_equal = torch.sum(equality).item()
		#
		# # 计算总元素数量
		# total_elements = torch.numel(weights)  # .item()用于将tensor的单个元素转换为Python中的一个普通整数
		#
		# # 计算相同元素的百分比
		# percentage_equal = (num_equal / total_elements) * 100
		#
		# print(f"相同元素的百分比: {percentage_equal:.2f}%")
		return tra_weight
