...
class VGG_3d(nn.Module):
	...
	def forward(self, x,deploy = "0"):
		x = self.conv[:3](x)
		if deploy == "1":
			x = self.conv[4:6](self.ture_conv(x, self.conv[3:6], self.param[1]))
		elif deploy == "2":
			x = self.conv[3:6](x)
		else:
			x = self.conv[4:6](self.ture_conv(x, self.conv[3:6], self.param[1]))
		...
		x = self.conv[7:10](x)
		if deploy == "1":
			x = self.conv[11:13](self.ture_conv(x, self.conv[10:13], self.param[3]))
		elif deploy == "2":
			x = self.conv[10:13](x)
		else:
			x = self.conv[11:13](self.ture_conv(x, self.conv[10:13], self.param[3]))
		...
		x = self.conv[14:17](x)
		if deploy == "1":
			x = self.conv[18:20](self.ture_conv(x, self.conv[17:20], self.param[5]))
		elif deploy == "2":
			x = self.conv[17:20](x)
		else:
			x = self.conv[18:20](self.ture_conv(x, self.conv[17:20], self.param[5]))
		...
		if deploy == "1":
			x =self.conv[21:23](self.ture_conv(x, self.conv[20:23], self.param[7]))
		elif deploy == "2":
			x = self.conv[20:23](x)
		else:
			x = self.conv[21:23](self.ture_conv(x, self.conv[20:23], self.param[7]))
		...
		x = self.conv[24:27](x)
		if deploy == "1":
			x = self.conv[28:30](self.ture_conv(x, self.conv[27:30], self.param[9]))
		elif deploy == "2":
			x = self.conv[27:30](x)
		else:
			x = self.conv[28:30](self.ture_conv(x, self.conv[27:30], self.param[9]))
		...
		if deploy == "1":
			x = self.conv[31:33](self.ture_conv_conv(x, self.conv[30:33], self.param[11],self.conv[35:43]))
		elif deploy == "2":
			x = self.conv[30:33](x)
		else:
			x = self.conv[31:33](self.ture_conv_conv(x, self.conv[30:33], self.param[11],self.conv[35:43]))
		...
		# if deploy == "1":
		# 	x = self.conv[35:37](self.ture_conv(x, self.conv[34:37], self.param[13]))
		# elif deploy == "2":
		# 	x = self.conv[34:37](x)
		# else:
		# 	x = self.conv[35:37](self.ture_conv(x, self.conv[34:37], self.param[13]))
		...
		# if deploy == "1":
		# 	x = self.conv[38:40](self.ture_conv(x, self.conv[37:40], self.param[15]))
		# elif deploy == "2":
		# 	x = self.conv[37:40](x)
		# else:
		# 	x = self.conv[38:40](self.ture_conv(x, self.conv[37:40], self.param[15]))
		...
		# if deploy == "1":
		# 	x = self.conv[41:43](self.ture_conv(x, self.conv[40:43], self.param[17]))
		# elif deploy == "2":
		# 	x = self.conv[40:43](x)
		# else:
		# 	x = self.conv[41:43](self.ture_conv(x, self.conv[40:43], self.param[17]))
	...
	def _qu_weight_conv(self,conv,all_conv):
		weights = conv[0].weight
		tra_weight = all_conv(weights)
	...
	def ture_conv(self,input,conv,param):
		x = F.conv2d(input, param * self._qu_weight(conv), bias=self._qu_bias(conv), stride=1, padding=1) # 第一次卷积操作
		return x

	def ture_conv_conv(self,input,conv,param,all_conv):
		x = F.conv2d(input, param * self._qu_weight_conv(conv,all_conv), bias=self._qu_bias(conv), stride=1, padding=1) # 第一次卷积操作
		return x
