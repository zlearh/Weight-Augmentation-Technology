...
class MBConvBlock(nn.Module):
    ...
    def _qu_weight(self, conv, radom):
        ...
        # 	])
            ...
            transform = transforms.Compose([
                ...
                transforms.RandomResizedCrop((weights.shape[-1], weights.shape[-1]), scale=(0.8, 1.0))
                # transforms.RandomRotation(90),  # 随机旋转（-10度到+10度之间）
            ])
        weights = transform(weights)
        return weights

    def ture_depthwise(self, input, conv, param, stride, radom):
        x = F.conv2d(input, param * self._qu_weight(conv, radom), bias=self._qu_bias(conv), stride=stride,
                     padding=(conv.weight.shape[-1] - 1) // 2,groups=input.size(1))  # 第一次卷积操作
        return x
    ...
    def forward(self, x, drop_connect_rate=None):
        ...
        if deploy == "1":
            x = self.ture_depthwise(x, self._depthwise_conv, self.param[0], self.stride, radom=random.choice([1,2]))
...
def build_efficientnet_lite():
    width_coefficient, depth_coefficient, _, dropout_rate = efficientnet_lite_params['efficientnet_lite0']
    model = EfficientNetLite(width_coefficient, depth_coefficient, 100, 0.2, dropout_rate)
    return model
